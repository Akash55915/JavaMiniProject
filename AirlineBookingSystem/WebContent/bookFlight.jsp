...
<html>
  ...
  url="jdbc:mysql://localhost:3306/akshay" user="root" password="root" />
   ...
   <c:when test="${not empty param.id  && not empty param.name && not empty param.price}">       
    ...
    <br/><br/>
    <% 
    String flights_select_source=(String)session.getAttribute("flights_select_source");
    String flights_select_destination=(String)session.getAttribute("flights_select_destination");%>
    <sql:query dataSource="${myDS}" var="pro">
        select * from flight where flight_source="${ flights_select_source}" && flight_destination="${ flights_select_destination}";
    </sql:query>
...
<%-- flight_total_no_seats:<c:out value="${pro.flight_No}" /> --%><br>
no_of_seats:<input type="number"  name="no_of_seats"/><br>
